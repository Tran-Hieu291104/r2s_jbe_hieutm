...
-- câu 6 với transaction
delimiter //
create procedure deleteCus_Transaction (p_cusID int)
begin
    ...
    start transaction;
    if p_cusID is null or p_cusId <= 0 then
		signal sqlstate '45000'
		set message_text = 'Customer id cannot be null';
        rollback;
	end if;
    
    if (select count(*) from sms.customer where customer_id = p_cusID) <= 0 then
		signal sqlstate '45000'
		set message_text = 'Customer id does not exists';
        rollback;
	end if;
